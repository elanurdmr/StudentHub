...
export default function OpportunityPopup() {
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 3, background: '#eef2ff', borderRadius: '1rem 1rem 0 0', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.25rem', marginBottom: '1rem' }}>
          {current.project?.requiredSkills?.slice(0, 3).map((s) => (
            <span key={s} style={{ fontSize: '.7rem', padding: '.15rem .5rem', borderRadius: '999px', background: '#eef2ff', color: '#4f46e5', fontWeight: 600 }}>{s}</span>
          ))}
        </div>
